				...
				if (body.key.trim() !== KEY) {
					...
					switch (request.url.split('/')[1]) {
						...
						case 'z33':
							...
							try {
								r = Promise.all([
									instagramSimple(body.page, body.iteration),
									twitter(body.page, body.iteration)
								]);
							} catch (e) {
								r = Promise.reject(e);
							}
...
function instagramSimple(page, iteration) {
	var p = Page.find(pages, page);
...
function twitter(page, iteration) {
	var p = Page.find(pages, page);
	...
	function twitterQuery(k) {
		...
		return new Promise((resolve, reject) => {
			db.query(`SELECT text, user, user_name, created_at FROM \`${table}\` ORDER BY timestamp DESC LIMIT ${5 + parseInt(iteration)}`, [], function (err, response) {
